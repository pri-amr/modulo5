...
import { MoneySourceModel, type MoneySourceDocument } from '../models/MoneySourceModel';

const MONEY_SOURCE_NOT_FOUND_MESSAGE = 'MoneySource no encontrada al recalcular el balance';
...
export class MoneySourceRepository implements IMoneySourceRepository {
  ...
  async incrementAmount(id: string, currency: 'ARS' | 'USD', delta: number): Promise<void> {
    if (!Types.ObjectId.isValid(id)) {
