...
const buildTestApp = (): Application => {
  ...
  app.get('/validation', (_req: Request, _res: Response, next: NextFunction) => {
  ...
  app.get('/uncontrolled', () => {
    throw new Error('Detalle interno sensible: query de Mongo, stack trace, etc.');
  });
...
describe('errorHandler', () => {
  ...
  it('devuelve el statusCode y el mensaje de un CustomError lanzado (ValidationError)', async () => {
    ...
    expect(response.body).toEqual({ error: 'El campo amount es requerido' });
  });
