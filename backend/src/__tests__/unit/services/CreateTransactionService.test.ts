...
const buildMoneySource = (
    overrides: Partial<MoneySource> = {}
): MoneySource => ({
    id: "money-source-1",
    userId: USER_ID,
...
const buildService = (
    ...
    const moneySources = new Map<string, MoneySource>([
        [moneySource.id, moneySource]
    ]);
    const categories = new Map<string, Category>([[category.id, category]]);
    const transactionRepository = new FakeTransactionRepository();
    const moneySourceRepository = new FakeMoneySourceRepository(
        moneySources,
        incrementAmountImpl
    );
...
describe("CreateTransactionService", () => {
    ...
    it("rechaza sin amount", async () => {
        ...
        await expect(
            service.execute(USER_ID, dtoWithoutAmount)
    ...
    it("rechaza amount <= 0", async () => {
        ...
        await expect(
            service.execute(USER_ID, { ...validDto, amount: 0 })
        ).rejects.toThrow(ValidationError);
        await expect(
            service.execute(USER_ID, { ...validDto, amount: -10 })
        ).rejects.toThrow(ValidationError);
    });
    ...
    it.each(["moneySourceId", "currency", "categoryId", "date", "description"])(
        ...
        async (field) => {
            ...
            await expect(
                service.execute(USER_ID, incompleteDto)
            ).rejects.toThrow(ValidationError);
        }
    );
    ...
    it("si incrementAmount falla después de crear la transacción, revierte la transacción y propaga el error", async () => {
        const moneySource = buildMoneySource();
        const category = buildCategory();
        const failingIncrement = (): Promise<void> =>
            Promise.reject(new Error("Fallo de balance"));
        ...
        await expect(service.execute(USER_ID, validDto)).rejects.toThrow(
            "Fallo de balance"
        );
