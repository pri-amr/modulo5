...
const tailwindPackagePath = createRequire(__filename).resolve("tailwindcss/package.json");
const tailwindPackage = JSON.parse(readFileSync(tailwindPackagePath, "utf-8")) as TailwindPackage;

const TAILWIND_ROOT = path.dirname(tailwindPackagePath);
const TAILWIND_ENTRY = path.resolve(TAILWIND_ROOT, tailwindPackage.exports["."].style);
...
const loadStylesheet = async (
  ...
  return { path: target, base: path.dirname(target), content: readFileSync(target, "utf-8") };
};
...
const extractWidthMedia = (css: string): WidthMedia[] =>
  extractMediaBlocks(css).reduce<WidthMedia[]>((acc, block) => {
    ...
    return match ? [...acc, { condition: block.condition, width: match[1].trim() }] : acc;
  }, []);
...
describe("AuthLayout", () => {
  ...
  it("test-block4-card-width-base-and-cap", () => {
    ...
    expect(screen.getByTestId("auth-layout-card")).toHaveClass("w-full", "max-w-4xl");
  });
  ...
  it("test-block4-card-width-cascade-order", async () => {
    ...
    const cardClasses = screen.getByTestId("auth-layout-card").className.split(/\s+/).filter(Boolean);
    const widthMedia = extractWidthMedia(await compileCardClasses(cardClasses));
    ...
    expect(emittedWidths.indexOf("40%")).toBeGreaterThan(emittedWidths.indexOf("70%"));
