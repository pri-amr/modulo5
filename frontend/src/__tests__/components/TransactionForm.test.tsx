...
const mockedCreateTransaction = TransactionService.createTransaction as jest.Mock;

const MONEY_SOURCE_OPTIONS: SelectOption[] = [{ value: "money-source-test-id", label: "Efectivo" }];
const CATEGORY_OPTIONS: SelectOption[] = [{ value: "category-test-id", label: "General" }];

const renderForm = (): ReturnType<typeof render> =>
  render(<TransactionForm moneySourceOptions={MONEY_SOURCE_OPTIONS} categoryOptions={CATEGORY_OPTIONS} />);
...
const fillForm = (overrides: FieldFillOverrides = {}): void => {
  ...
  (Object.keys(fields) as Array<keyof FieldFillOverrides>).forEach((field) => {
...
describe("TransactionForm", () => {
  ...
  it("completa un ingreso válido y confirma → llama a TransactionService con el DTO correcto (AC-02)", async () => {
    ...
    renderForm();
    fillForm({ type: "ingreso", amount: "500", currency: "USD", date: "15-01-2026", description: "Sueldo" });
    submitForm();
  ...
  it("enviar sin monto → error de validación visible, no se llama al service (AC-04)", async () => {
    ...
    expect(await screen.findByText("El monto es requerido")).toBeInTheDocument();
  ...
  it("enviar sin fuente/moneda/categoría/fecha/descripción (uno por caso) → error de validación visible (AC-06)", async () => {
    const casesMissingOneField: Array<{ overrides: FieldFillOverrides; expectedError: string }> = [
      { overrides: { moneySourceId: "" }, expectedError: "La fuente de dinero es requerida" },
      { overrides: { currency: "" }, expectedError: "Seleccioná una moneda válida" },
      { overrides: { categoryId: "" }, expectedError: "La categoría es requerida" },
      { overrides: { date: "" }, expectedError: "La fecha es requerida" },
      { overrides: { description: "" }, expectedError: "La descripción es requerida" },
    ];
  ...
  it("enviar con fecha en formato distinto a DD-MM-YYYY → error de validación visible (AC-09)", async () => {
    ...
    expect(await screen.findByText("La fecha debe tener el formato DD-MM-YYYY")).toBeInTheDocument();
    expect(mockedCreateTransaction).not.toHaveBeenCalled();
  });
  ...
  it("el service devuelve 403 → el hook expone el error y el formulario conserva los datos ingresados (AC-07, AC-08)", async () => {
    ...
    expect(
      await screen.findByText("La fuente de dinero no existe o no pertenece al usuario"),
    ).toBeInTheDocument();
    expect(screen.getByLabelText("Descripción")).toHaveValue(VALID_FIELDS.description);
    expect(screen.getByLabelText("Monto")).toHaveValue(Number(VALID_FIELDS.amount));
  });

  it("el service devuelve 500 → el hook expone el error y el formulario conserva los datos ingresados (AC-08)", async () => {
    mockedCreateTransaction.mockRejectedValueOnce(new Error("Internal server error"));
