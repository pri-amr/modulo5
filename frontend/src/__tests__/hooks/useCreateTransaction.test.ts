...
import { useCreateTransaction, type TransactionFormValues } from "@/hooks/useCreateTransaction";
  ...
  it("aborta la request en curso al desmontarse y no actualiza estado del componente desmontado", async () => {
    ...
    act(() => {
      Object.entries(VALID_VALUES).forEach(([field, value]) => {
        result.current.setFieldValue(field as keyof TransactionFormValues, value);
      });
    });
