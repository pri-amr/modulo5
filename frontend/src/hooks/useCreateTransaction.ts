...
import type { Currency, CreateTransactionRequestDto, TransactionType } from "@/models/Transaction.types";
...
export type TransactionFormFieldErrors = Partial<Record<keyof TransactionFormValues, string>>;
...
const transactionFormSchema = yup.object({
  ...
  moneySourceId: yup.string().trim().required("La fuente de dinero es requerida"),
  currency: yup
    .mixed<Currency>()
  ...
  date: yup
    ...
    .test("es-fecha-real", "La fecha no corresponde a una fecha real", isRealDate),
  description: yup
    .string()
    .trim()
...
export const useCreateTransaction = (): UseCreateTransactionResult => {
  ...
  const setFieldValue = useCallback((field: keyof TransactionFormValues, value: string): void => {
  ...
  const submit = useCallback(async (): Promise<void> => {
    ...
    try {
      validated = await transactionFormSchema.validate(values, { abortEarly: false });
    } catch (validationError) {
      if (validationError instanceof yup.ValidationError) {
        const nextFieldErrors: TransactionFormFieldErrors = {};

        validationError.inner.forEach((issue) => {
          const path = issue.path as keyof TransactionFormValues | undefined;
